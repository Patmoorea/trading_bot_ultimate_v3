...
class TestTriangularArbitrage:
    ...
    async def test_triangular_opportunity_detection(self, mock_market_data):
        ...
        final_amount = (eth_amount * eth_usdc_price).quantize(Decimal('0.00000001'))

        profit_ratio = float(final_amount / initial_amount)
        assert profit_ratio > 1.001, f"L'opportunité d'arbitrage devrait être profitable (ratio: {profit_ratio})"
