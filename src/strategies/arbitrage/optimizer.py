...
class CrossExchangeOptimizer:
    def __init__(self, brokers):
        ...
        self.constraints = {
            ...
            "blofin": BrokerConstraint(quote_asset="USD", fee=0.0005, min_volume=0.005),
        }

    async def get_optimal_pair(self, base_asset: str) -> Dict:
        ...
        for broker_name, broker in self.brokers.items():
            ...
            try:
                orderbook = await broker.fetch_order_book(symbol)
                effective_bid = (
                    orderbook["bids"][0][0]
                    * constraint.adjustment_factor
                    * (1 - constraint.fee)
                )
