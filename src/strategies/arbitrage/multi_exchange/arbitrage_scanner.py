...
class ArbitrageScanner:
    def __init__(
        self,
        exchanges=None,
        min_profit=0.002,               # Changé de min_profit_threshold à min_profit
        ...
        self.min_profit = min_profit    # Changé ici aussi
    ...
    async def scan_opportunities(self) -> list[dict]:
        ...
        try:
            ...
            logger.error(f"[2025-06-09 01:54:26] Erreur dans scan_opportunities: {e}")
    ...
    async def check_price_feasibility(self, opportunity: dict) -> bool:
        ...
        try:
            ...
            buy_ex = next(e for e in self.exchanges if e.name == opportunity["buy_exchange"])
            sell_ex = next(e for e in self.exchanges if e.name == opportunity["sell_exchange"])
            ...
            # Calcul du slippage
            buy_slippage = abs(buy_ticker['ask'] - opportunity["buy_price"]) / opportunity["buy_price"]
            sell_slippage = abs(sell_ticker['bid'] - opportunity["sell_price"]) / opportunity["sell_price"]
            
            return max(buy_slippage, sell_slippage) <= self.max_slippage
            
        except Exception as e:
            logger.error(f"[2025-06-09 01:54:26] Erreur dans check_price_feasibility: {e}")
            return False
    ...
    async def find_opportunities(self) -> list[dict]:
        ...
        try:
            ...
            logger.error(f"[2025-06-09 01:54:26] Erreur dans find_opportunities: {e}")
            return []
