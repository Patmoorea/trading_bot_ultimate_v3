...
class ArbitrageEngine:
    ...
    @property
    def brokers(self):
        return self.exchange_manager.exchanges
        ...
        for symbol in PAIRS:
            ...
            for exchange, pair in PAIRS[symbol].items():
                ...
                if bid > 0 and ask < Decimal("Infinity"):
                    ...
                    logger.debug(
                        f"{symbol} prices on {exchange}: bid={bid:.8f}, ask={ask:.8f}"
                    )
            if len(prices) < 2:
                continue
            best_bid_exchange, (best_bid, _) = max(
                prices.items(), key=lambda x: x[1][0]
            )
            best_ask_exchange, (_, best_ask) = min(
                prices.items(), key=lambda x: x[1][1]
            )
