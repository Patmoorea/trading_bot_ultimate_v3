...
Last updated: 2025-05-19 23:16:17 UTC
"""
...
class GateIOExchange:
    ...
    async def _load_min_amounts(self) -> None:
        ...
        for symbol, market in self._exchange.markets.items():
            self._min_trade_amounts[symbol] = market.get('limits', {}).get('amount', {}).get('min')
    ...
    async def get_arbitrage_opportunities(self, symbols: List[str], min_profit_percent: float = 0.1) -> List[Dict]:
        """
    ...
    async def execute_arbitrage(self, opportunity: Dict) -> Dict:
        ...
        try:
            ...
            amount = max(min_amount, (usdt_balance * 0.1) / opportunity['ask_price'])  # Utilise 10% du solde
            ...
            buy_order = await self.create_order(
                ...
                side="buy",
                amount=str(amount),
                price=str(opportunity['ask_price'] * 1.001)  # Légèrement au-dessus du ask
            )
            ...
            sell_order = await self.create_order(
                ...
                side="sell",
                amount=str(amount),
                price=str(opportunity['bid_price'] * 0.999)  # Légèrement en-dessous du bid
            )
