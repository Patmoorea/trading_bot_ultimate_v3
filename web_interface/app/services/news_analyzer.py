...
class NewsAnalyzer:
    ...
    async def analyze_news(self) -> Dict:
        try:
            ...
            if not news:
                return {"status": "error", "reason": "Aucune news récupérée sur aucune source"}
            ...
            important_news = self._filter_important_news(news, sentiment_results, impact_scores)
    ...
    async def _fetch_cryptopanic_news(self) -> List[Dict]:
        if not CRYPTO_PANIC_API_KEY: return []
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        self.logger.error(f"Erreur CryptoPanic: Status code {resp.status}")
        except Exception as e:
    ...
    async def _fetch_newsapi_news(self) -> List[Dict]:
        ...
        try:
            ...
            params = {
                "apiKey": NEWS_API_KEY,
                "language": NEWS_API_LANGUAGES[0],  # NewsAPI ne prend qu'une langue à la fois
                "pageSize": 50,
            }
    ...
    def _filter_important_news(self, news, sentiment_results, impact_scores) -> List[Dict]:
        important = []
        for r in sentiment_results:
    ...
    def _generate_summary(self, sentiment_results: List[Dict]) -> Dict:
        ...
        sentiment_counts = {k: sentiments.count(k) for k in ['positive', 'neutral', 'negative']}
