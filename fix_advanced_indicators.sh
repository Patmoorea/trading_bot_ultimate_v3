...
mkdir -p src/analysis/technical/advanced

# Création du fichier advanced_indicators.py avec la bonne indentation
cat > src/analysis/technical/advanced/advanced_indicators.py << 'EOF'
import pandas as pd
import numpy as np
...
class AdvancedIndicators:
    ...
    def _kama(self, data: pd.DataFrame, period: int = 20, fast: int = 2, slow: int = 30) -> pd.Series:
...
chmod +x fix_advanced_indicators.sh
./fix_advanced_indicators.sh

echo "Le fichier advanced_indicators.py a été corrigé!"
echo "Redémarrez l'application avec: streamlit run src/main.py --server.fileWatcherType none"
