...
@pytest.mark.asyncio
async def test_find_triangular_opportunities(config):
    arb = TriangularArbitrage(config)
    arb.moteur = DummyMoteur(config)  # Patch le moteur mock
...
@pytest.mark.asyncio
async def test_calculate_path_profit(config):
    arb = TriangularArbitrage(config)
    arb.moteur = DummyMoteur(config)
    pairs = ("BTC", "ETH", "USDT")
    orderbooks = {
